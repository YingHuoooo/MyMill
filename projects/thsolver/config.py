...
_C.SOLVER.trainable_keywords = ''        # Comma-separated trainable name filters
...
_C.SOLVER.visualize         = False      # Save point visualization obj files
...
_C.MODEL.conditioning       = 'concat'    # concat, film, film_skip, or film_both
_C.MODEL.film_scale         = 1.0         # Residual scale for FiLM modulation
...
_C.CALIB.cp_method          = 'threshold' # threshold or aps prediction sets
_C.CALIB.crc_alpha          = 0.05        # CRC false-negative risk level
_C.CALIB.red_crc_alpha      = 0.03        # CRC risk level for red head
_C.CALIB.green_crc_alpha    = 0.03        # CRC risk level for green head
_C.CALIB.crc_max_threshold  = 1.0         # Set 0.5 to avoid stricter-than-argmax CRC
_C.CALIB.risk_rescue        = False       # Selectively rescue near-boundary risk points
_C.CALIB.risk_rescue_budget = 0.002       # Max extra predicted-risk rate per head
_C.CALIB.risk_rescue_min_prob = 0.25      # Lowest risk probability eligible for rescue
...
_C.CALIB.temperature_steps  = 91          # Number of grid-search values
_C.CALIB.calibration_baselines = False    # Evaluate stronger TS baselines
_C.CALIB.calibration_baseline_methods = 'local_temp,parameterized_temp,adaptive_temp'
_C.CALIB.local_temperature_bins = 2       # Spatial bins per xyz axis for local TS
_C.CALIB.temperature_fit_max_points = 200000  # Max calibration points for learned TS
_C.CALIB.parameterized_temperature_steps = 120
_C.CALIB.adaptive_temperature_steps = 120
_C.CALIB.fixed_threshold    = False       # Evaluate fixed risk threshold baseline
_C.CALIB.fixed_red_threshold = 0.45       # Fixed risk threshold for red head
_C.CALIB.fixed_green_threshold = 0.45     # Fixed risk threshold for green head
_C.CALIB.calibrated_fixed_threshold = False  # Select per-head thresholds on calibration split
_C.CALIB.calibrated_fixed_thresholds = (0.30, 0.35, 0.40, 0.45, 0.50)
_C.CALIB.calibrated_fixed_budget = 0.002  # Max extra predicted-risk rate per head
_C.CALIB.adaptive_crc       = False       # Calibrate CRC by difficulty bins
_C.CALIB.adaptive_crc_bins  = 2           # Number of adaptive CRC bins
_C.CALIB.adaptive_crc_score = 'entropy'   # entropy, confidence, or margin
